export default {
  calculatePaging(requestPagingOptions: any, count: number) {
    let offset: number =
      requestPagingOptions.perPage * (requestPagingOptions.page - 1);
    let lastPage: number = this.roundUp(
      count / requestPagingOptions.perPage,
      0,
    );
    let lastPageOffset: number = requestPagingOptions.perPage * (lastPage - 1);
